...
namespace Input {
    ...
    /// time step size
    ///
    constexpr Real dt = 0.0004;

    /// number of time steps for inner loop
    /// total time step Nt = inner_Nt * outer_Nt
    /// simulation time t = dt*Nt
    ///
    constexpr unsigned inner_Nt = 250;

    /// number of time steps for outer loop
    /// total time step Nt = inner_Nt * outer_Nt
    /// simulation time t = dt*Nt
    ///
    constexpr unsigned outer_Nt = 1500;
    ...
    std::make_tuple(BiMaxPlasmaDesc({{  1,  25, 2}, 10000, full_f},   5, .1),
                    BiMaxPlasmaDesc({{-25, 125, 2}, 10000, full_f}, 0.5,  1)
                    );
