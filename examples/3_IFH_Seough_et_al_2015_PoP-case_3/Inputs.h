...
namespace Input {
    ...
    /// time step size
    ///
    constexpr Real dt = 0.00025;

    /// number of time steps for inner loop
    /// total time step Nt = inner_Nt * outer_Nt
    /// simulation time t = dt*Nt
    ///
    constexpr unsigned inner_Nt = 400;
    ...
    std::make_tuple(BiMaxPlasmaDesc({{  1,  40, 2}, 10000, full_f}, 10, .1),
                    BiMaxPlasmaDesc({{-25, 200, 2}, 10000, full_f},  1,  1)
                    );
